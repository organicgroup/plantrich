...
class ExportTraces(Document):
	def validate(self):
		self.create_qr_code()

	def create_qr_code(self):
		if not self.eudr_qr_code:
			print_format = 'EUDR Print Format'
			qr_image = io.BytesIO()
			...
			filename = f"EUDR_QRCode-{name}.png".replace(os.path.sep, "__")
			_file = frappe.get_doc({
				"doctype": "File",
				"file_name": filename,
				"attached_to_doctype": self.get("doctype"),
				"attached_to_name": self.get("name"),
				"attached_to_field": "eudr_qr_code",
				"is_private": 0,
				"content": qr_image.getvalue()
			})
			_file.save(ignore_permissions=True)
			self.db_set('eudr_qr_code', _file.file_url)